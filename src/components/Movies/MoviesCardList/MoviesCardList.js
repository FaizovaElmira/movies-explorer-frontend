...
function MoviesCardList({ movies }) {
  ...
  const renderMovies = () => {
    return movies.slice(0, visibleMoviesCount).map((card) => (
  ...
  const shouldRenderShowMoreButton = location.pathname === '/movies' && visibleMoviesCount < movies.length;
  ...
  return (
    <section className={containerClass}>
      ...
      {shouldRenderShowMoreButton && (
        <button className="moviesCardList__showMore" onClick={handleShowMoreClick}>Ещё</button>
      )}
    </section>
  );
}
